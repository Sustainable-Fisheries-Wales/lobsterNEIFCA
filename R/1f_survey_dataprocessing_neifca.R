...
colnames(survey_data_effort) <- survey_data_effort |> 
  janitor::clean_names() |> 
  colnames() 
survey_data_crab <- readr::read_csv(file = "data/neifca/raw/potting.survey_crab.csv") |> 
  dplyr::rename(carapace_len=CL) |> dplyr::glimpse() 
colnames(survey_data_crab) <- survey_data_crab |> 
  janitor::clean_names() |> 
  colnames() 
...
readr::write_csv(survey_data_crab, file = "processed_data/neifca/survey_data_crab_neifca_clean.csv") 
