# script to reformat size composition data as SS model input for the crab and lobster quayside data - North Eastern IFCA 
# created: 8/30/2024 by Daisuke Goto (d.goto@bangor.ac.uk)
...
quayside_data_lobster <- readr::read_csv("processed_data/neifca/quayside_data_ind_lobster_neifca_clean.csv") |> 
  ...
  dplyr::glimpse()
quayside_data_crab <- readr::read_csv("processed_data/neifca/quayside_data_ind_crab_neifca_clean.csv") |> 
  dplyr::filter(!is.na(date)) |>
...
size.max <- 21#round(max(data$length, na.rm = TRUE)) 20
width <- 0.2
...
size.dist_lobster_f <- size.dist_lobster |> 
  dplyr::filter(sex == 1) |> 
  dplyr::mutate(sex = 3) 
size.dist_lobster_quayside <- size.dist_lobster_f |> 
  dplyr::bind_cols(size.dist_lobster_m) 
colnames(size.dist_lobster_quayside) <- c("year", "month", "fleet", "sex", "part", "nsample", 
                                          paste0("f", 1:n.size), paste0("m", 1:n.size))

# crab
data <- quayside_data_crab
colnames(data)[4] <- "length"
data <- data |> dplyr::filter(!is.na(sex))
data$length <- data$length/10 # recorded in mm -> convert to cm for ss
(size.min <- round(min(data$length, na.rm = TRUE))) #1
...
size.dist_crab_f <- size.dist_crab |> 
  dplyr::filter(sex == 1) |> 
  dplyr::mutate(sex = 3) 
size.dist_crab_quayside <- size.dist_crab_f |> 
  dplyr::bind_cols(size.dist_crab_m) |> 
  dplyr::mutate(nsample = nsample+nsample.m) |>
  dplyr::select(-nsample.m)
colnames(size.dist_crab_quayside) <- c("year", "month", "fleet", "sex", "part", "nsample", 
                                       paste0("f", 1:n.size), paste0("m", 1:n.size))
...
# export data
readr::write_csv(size.dist_lobster_quayside, file = "processed_data/neifca/quayside.size.comp.data_lobster_neifca_ss.csv") 
readr::write_csv(size.dist_crab_quayside, file = "processed_data/neifca/quayside.size.comp.data_crab_neifca_ss.csv") 
